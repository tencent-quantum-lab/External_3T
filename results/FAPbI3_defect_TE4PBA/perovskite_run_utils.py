...
def pack_molecules(lattice_data, molecules_data):
    ...
    mass_elem_dict = {1:'H', 7:'Li', 12:'C', 14:'N', 16:'O', 19:'F', 31:'P', 127:'I', 207:'Pb'}
    with open('pack_in.inp', 'w') as f:
        ...
        for molecule_data in molecules_data:
            ...
            f.write('  number 1\n')
            # Force cation to be on top of defect site of FAPbI3
            f.write('  inside box 1 6 0 11 16 10' + '\n')
...
def create_optimizers(model, block):
    ...
    optim_params = [param for param in model.movable_pos_list]
    ligand_translation = model.translation_list
    ligand_rotation = model.rotation_list
    optim_params += [ligand_translation, ligand_rotation]
    if special_rotation != None:
        ligand_special_rotation = model.special_rotation_list
        optim_params += [ligand_special_rotation]
    if macro_mode != None:
        ligand_macro_translation = model.macro_mode_translation_list
        ligand_macro_rotation = model.macro_mode_rotation_list
